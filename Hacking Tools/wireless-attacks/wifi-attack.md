...
- [🔍 ความเสี่ยงของการเปลี่ยน MAC address และการตรวจสอบ](#-ความเสี่ยงของการเปลี่ยน-mac-address-และการตรวจสอบ)
...
- [🕵️ การดักจับ Handshake](#-การดักจับ-handshake)
- [💥 การยิง deauth (ไล่ client ออกเพื่อดัก handshake)](#-การยิง-deauth-ไล่-client-ออกเพื่อดัก-handshake)
- [✅ ตรวจสอบ Handshake ด้วย aircrack-ng](#-ตรวจสอบ-handshake-ด้วย-aircrack-ng)
...
## 🔍 ความเสี่ยงของการเปลี่ยน MAC address และการตรวจสอบ

แม้จะใช้ `macchanger` เปลี่ยน MAC ได้สำเร็จ แต่ **MAC Address จริงยังคงอยู่ในฮาร์ดแวร์ของอะแดปเตอร์** ซึ่งระบบบางประเภท เช่น IDS/IPS หรืออุปกรณ์เครือข่ายระดับองค์กร อาจสามารถตรวจสอบย้อนกลับได้

### ✅ วิธีเปลี่ยน MAC อย่างปลอดภัย:
...
### ✅ ตรวจสอบ MAC Address จริง (เครื่องเราเท่านั้น):

```bash
ethtool -P wlan1
lshw -class network | grep -i serial
```

### ✅ เช็คว่าปลอมสำเร็จไหม:

```bash
ip link show wlan1
```

### 🧠 เทคนิค Red Team เพิ่มเติม:

| เทคนิค | รายละเอียด |
|--------|-------------|
| 💾 ใช้ Live USB / VM | หลีกเลี่ยงหลุด MAC จริงจาก Host |
| 🔇 ปิด NetworkManager | ป้องกันการ broadcast packet อัตโนมัติ |
| 🔁 เปลี่ยน MAC บ่อย | เพื่อสับขาหลอก trace |
| 🔍 เช็คด้วย Wireshark | ดูว่า packet ที่ส่งออกมี MAC อะไรแน่ |
...
## ✅ ตรวจสอบ Handshake ด้วย `aircrack-ng`

ก่อนแปลง `.cap` เป็น `.22000` หรือเริ่ม brute-force ควรเช็คก่อนว่าเราดัก **WPA Handshake** ได้สำเร็จหรือยัง
...
- หากมีบรรทัดนี้แสดงว่าได้ Handshake แล้ว:
  ```
  WPA handshake: XX:XX:XX:XX:XX:XX
  ```
- หากไม่ขึ้น → ยังไม่ได้ Handshake → ต้องรอดักใหม่หรือลองยิง deauth เพิ่ม
- ใช้ตรวจสอบไฟล์ `.cap` ได้ทันทีโดยไม่ต้องแคร็กจริง
