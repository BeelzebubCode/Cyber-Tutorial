...
## 🔐 ตรวจสอบว่าได้ Hash แล้วหรือยัง (ด้วย aircrack-ng)

หลังจากดักจับ handshake แล้ว สามารถใช้ `aircrack-ng` ตรวจสอบว่าไฟล์ `.cap` มี handshake จริงหรือไม่:

```bash
aircrack-ng -a2 -w /dev/null capture.cap
```

หากมี handshake จริง จะขึ้นว่า:

```
WPA handshake: [BSSID] (station MAC)
```

📌 `-w /dev/null` ใช้เพื่อไม่ต้อง brute-force จริง แค่เช็ค handshake
